...
const Cart = (props) => {
    ...
    const addMore = () => {
        props.history.push("/all-product-to-sold")
    }
    ...
    const proceedToBuy = () => {
        ...
        if (status === true) {
            if (userAddress !== null) {
                ...
                // window.localStorage.setItem("order_date", orderDate);
                        ...
                        <div className="card-body mt-n5">
                                                ...
                                                cart.map(
                                                    product =>
                                                        <tr key={product.id}>
                                                            ...
                                                            <td>{product.cartPerDay}</td>
                                                            <td>{product.offerDiscount}</td>
                                                            <td>{product.finalCart}</td>
                                                            <td>
                            ...
                            {JSON.parse(window.localStorage.getItem("cart_size")) > 0 &&
                                <div className="card float-right mt-3">
                                    ...
                                    <h5 className="text-right font-weight-bold ml-4 mr-4">Saving Amount :  <i class="fas fa-minus-circle"></i>{cartTotalSavingAmt}</h5>
